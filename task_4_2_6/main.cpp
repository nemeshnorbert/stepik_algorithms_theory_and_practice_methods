...
huffman_code encode(Iter first, Iter last) {
  ...
  return huffman_code{encoded, table};
}
...
huffman_decode decode(
  ...
  auto ibit = std::begin(encoded);
  auto end = std::end(encoded);
  while (ibit != end) {
    if (node == nullptr) {
      return huffman_decode{"", false};
    }
    if (*ibit == '0') {
      node = node->get_left();
      ++ibit;
  ...
  return huffman_decode{decoded, true};
}

huffman_decode decode(const huffman_code& code) {
  return decode(code.encoded, code.table);
}

} // namespace huffman

int main() {
  auto symbols_count = std::size_t{0};
  auto encoded_text_size = std::size_t{0};
  std::cin >> symbols_count >> encoded_text_size;
  auto table = std::unordered_map<char, std::string>{};
  for (auto idx = std::size_t{0}; idx < symbols_count; ++idx) {
    auto symbol_description = std::string{};
    std::cin >> symbol_description;
    auto symbol_code = std::string{};
    std::cin >> symbol_code;
    table[symbol_description[0]] = symbol_code;
  }
  auto encoded = std::string{};
  std::cin >> encoded;
  auto decode_result = huffman::decode(encoded, table);
  std::cout << decode_result.decoded;
  return 0;
}